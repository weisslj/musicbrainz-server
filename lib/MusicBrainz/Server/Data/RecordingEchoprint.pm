...
sub get_by_recording_echoprint
{
    ...
    my $row = $self->sql->select_single_row_hash($query, $recording_id, $echoprint_str)
        or return;
...
sub merge_recordings
{
    ...
    $self->sql->do('DELETE FROM recording_echoprint
              WHERE recording IN ('.placeholders(@ids).')
                AND id NOT IN (
                SELECT DISTINCT ON (echoprint) id
                  FROM recording_echoprint
                 WHERE recording IN ('.placeholders(@ids).')
              )', @ids, @ids);

    # Move the rest
    $self->sql->do('UPDATE recording_echoprint SET recording = ?
...
sub delete_recordings
{
    ...
    my $echoprint_ids = $self->sql->select_single_column_array('
...
sub delete
{
    ...
    $self->sql->do($query, $recording_echoprint_id);
...
sub filter_additions
{
    ...
    my $query = 'SELECT v.* FROM
        (VALUES ' . join(', ', ('(?::integer, ?::integer, ?)') x @tuples) . ') AS v(echoprint, recording, name)
    ...
    my $rows = $self->sql->select_list_of_hashes(
        $query,
        map { $echoprints{ $_->{echoprint} }, $_->{recording}{id}, $_->{recording}{name} } @tuples
    );
    push @additions, map +{
